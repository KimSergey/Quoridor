...
for i in range(9):
    for j in range(9):
        if i == X1 and j == Y1 or i == X2 and j == Y2 or i == X3 and j == Y3 or i == X4 and j == Y4:
            places_squares[i][j] = Button(root, bg='#804020')
            places_squares[i][j].place(x=450+75*i, y=90+75*j, width=60, height=60)
        else:
...
for i in range(9):
    for j in range(8):
        ...
        spaces_horizontal[i][j].place(x=450+75*i, y=150+75*j, width=60, height=15)
for i in range(8):
    for j in range(9):
        spaces_vertical[i][j] = Button(root, bg='#800808')
        spaces_vertical[i][j].place(x=510+75*i, y=90+75*j, width=15, height=60)
can.pack()
